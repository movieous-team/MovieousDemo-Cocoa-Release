...
- (void)initalize {
    ...
    _progressIndicator.image = [UIImage imageNamed:@"progressbar_front"];
...
- (void)onTimer:(NSTimer *)timer
{
    [UIView animateWithDuration:TIMER_INTERVAL / 2 animations:^{
        self->_progressIndicator.alpha = 0;
    } completion:^(BOOL finished) {
        [UIView animateWithDuration:TIMER_INTERVAL / 2 animations:^{
            self->_progressIndicator.alpha = 1;
        }];
    }];
}
...
- (void)setLastProgressStyle:(MSVProgressBarProgressStyle)lastProgressStyle {
    _lastProgressStyle = lastProgressStyle;
    UIView *lastProgressView = [_progressViewArray lastObject];
    if (!lastProgressView) {
        return;
    }
    
    switch (lastProgressStyle) {
        case MSVProgressBarProgressStyleDelete:
        {
            lastProgressView.backgroundColor = [UIColor redColor];
            _progressIndicator.hidden = YES;
        }
            break;
        case MSVProgressBarProgressStyleNormal:
        {
...
- (void)deleteLastProgress {
    _lastProgressStyle = MSVProgressBarProgressStyleNormal;
    UIView *lastProgressView = [_progressViewArray lastObject];
    if (!lastProgressView) {
        return;
    }
