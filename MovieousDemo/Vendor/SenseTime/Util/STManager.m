...
- (void)setDefaultValue {
    ...
    self.detectResultLock = [[STLock alloc] init];
    
    self.microSurgeryModels = @[
                                getModel([UIImage imageNamed:@"zhailian"], [UIImage imageNamed:@"zhailian_selected"], @"瘦脸型", 0, NO, 0, STEffectsTypeBeautyMicroSurgery, STBeautyTypeNarrowFace),
                                ...
                                getModel([UIImage imageNamed:@"shoubiyi"], [UIImage imageNamed:@"shoubiyi_selected"], @"瘦鼻翼", 0, NO, 3, STEffectsTypeBeautyMicroSurgery, STBeautyTypeNarrowNose),
                                getModel([UIImage imageNamed:@"changbi"], [UIImage imageNamed:@"changbi_selected"], @"长鼻", 0, NO, 4, STEffectsTypeBeautyMicroSurgery, STBeautyTypeLengthNose),
                                getModel([UIImage imageNamed:@"zuixing"], [UIImage imageNamed:@"zuixing_selected"], @"嘴形", 0, NO, 5, STEffectsTypeBeautyMicroSurgery, STBeautyTypeMouthSize),
                                getModel([UIImage imageNamed:@"suorenzhong"], [UIImage imageNamed:@"suorenzhong_selected"], @"缩人中", 0, NO, 6, STEffectsTypeBeautyMicroSurgery, STBeautyTypeLengthPhiltrum),
                                ];
...
- (void)setupHandle {
    ...
    if (ST_OK != iRet || !_hDetector) {
        ...
        addSubModel(_hDetector, @"M_Align_Deepface_Iris_1.11.1");
#if TEST_BODY_BEAUTY
    ...
    if (ST_OK != iRet || !_hSticker) {
        ...
        NSString *strAvatarModelPath = [[NSBundle mainBundle] pathForResource:@"M_SenseME_Avatar_Core_1.0.0" ofType:@"model"];
    ...
    if (ST_OK != iRet || !_hBeautify) {
        ...
        //瘦脸型
        setBeautifyParam(_hBeautify, ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO, self.fNarrowFaceStrength);
...
- (CVPixelBufferRef)processPixelBuffer:(CVPixelBufferRef)pixelBuffer {
    ...
    if (_bTracker && _hTracker) {
        ...
        if (self.isCommonObjectViewAdded) {
            ...
            if (self.isCommonObjectViewSetted) {
                ...
                if ([self.delegate respondsToSelector:@selector(manager:commonObjectCenterDidUpdated:rect:)]) {
                    [self.delegate manager:self commonObjectCenterDidUpdated:center rect:_rect];
                }
            }
        }
    }
    ...
    if (_hDetector) {
        ...
        BOOL needFaceDetection = ((self.fEnlargeEyeStrength != 0 ||
                                   self.fShrinkFaceStrength != 0 ||
                                   self.fShrinkJawStrength != 0 ||
                                   self.fNarrowFaceStrength != 0 ||
                                   self.fChinStrength != 0 ||
                                   self.fHairLineStrength != 0 ||
                                   self.fNarrowNoseStrength != 0 ||
                                   self.fLongNoseStrength != 0 ||
                                   self.fMouthStrength != 0 ||
                                   self.fPhiltrumStrength != 0 ||
                                   self.fDehighlightStrength != 0) && _hBeautify) || (self.bAttribute && isAttributeTime && _hAttribute);
    ...
    if (_bSticker && _hSticker) {
        ...
        iRet = st_mobile_sticker_process_texture(_hSticker, textureResult, iWidth, iHeight, stMobileRotate, ST_CLOCKWISE_ROTATE_0, false, &newDetectResult, &inputEvent, _textureStickerOutput);
    ...
    if (self.isComparing) {
        ...
        resultPixelBufffer = pixelBuffer;
    }
        ...
        if (iRet != ST_OK) {
            ...
            if (0 != iAction) {//有 trigger信息
                ...
                if (CHECK_FLAG(iAction, ST_MOBILE_HAND_FINGER_INDEX)) {
                    ...
                    image = [UIImage imageNamed:@"two_index_finger"];
                }
...
- (void)handleFilterStrenthChange:(float)strenth {
    if (_hFilter) {
        ...
        st_result_t iRet = ST_OK;
        iRet = st_mobile_gl_filter_set_param(_hFilter, ST_GL_FILTER_STRENGTH, strenth);
