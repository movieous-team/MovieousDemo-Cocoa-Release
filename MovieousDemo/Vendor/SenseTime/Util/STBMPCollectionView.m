...
- (void)setupDefaults
{
    NSArray *types = @[@"腮红",@"眉毛",@"修容",@"口红",@"眼影",@"眼线",@"眼睫毛", @"美瞳"];
    NSArray *bmpTypes = @[@(STBMPTYPE_BLUSH),@(STBMPTYPE_BROW),@(STBMPTYPE_FACE),@(STBMPTYPE_LIP),@(STBMPTYPE_EYE),@(STBMPTYPE_EYELINER),@(STBMPTYPE_EYELASH), @(STBMPTYPE_EYEBALL)];
    NSArray *iconDefaults = @[@"blush",@"brow",@"face",@"lip",@"eyeshadow-white",@"eyeline-white",@"eyelash-white", @"eyeball"];
    NSArray *iconHightLights = @[@"blush_selected",@"brow_selected",@"face_selected",@"lip_selected",@"eyeshadow-purple",@"eyeline-purple",@"eyelash-purple",@"eyeball_select"];
