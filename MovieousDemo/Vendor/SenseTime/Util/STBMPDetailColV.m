...
@property (nonatomic, strong) NSMutableArray<STBMPModel *> *m_eyeballArr;
...
@property (nonatomic, assign) int m_eyeballIndex;
...
- (void)setupDefaults
{
    ...
    _m_eyeballArr = [self getSourceFromPath:@"eyeball" bmpType:STBMPTYPE_EYEBALL];
    
    _m_lipsIndex = _m_blushIndex = _m_browIndex = _m_eyeShadowIndex = _m_eyeLinerIndex = _m_eyeLashIndex = _m_faceIndex = _m_eyeballIndex = 0;
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (_m_bmpType) {
        ...
        case STBMPTYPE_EYEBALL:
            ...
            if ((int)indexPath.row == _m_eyeballIndex) {
                _m_eyeballIndex = -1;
                STBMPModel *unModel = [[STBMPModel alloc] init];
                unModel.m_bmpType = STBMPTYPE_EYEBALL;
                if (self.delegate) {
                    [self.delegate didSelectedModel:unModel];
                }
            }else{
                _m_eyeballIndex = (int)indexPath.row;
                if (self.delegate) {
                    [self.delegate didSelectedModel:model];
                }
            }
