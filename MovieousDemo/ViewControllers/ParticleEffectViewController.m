...
- (void)syncUIWithEffect {
    ...
    for (TuSDKMediaParticleEffectData *effect in [ShortVideoFilter.sharedInstance.filterProcessor mediaEffectsWithType:TuSDKMediaEffectDataTypeParticle]) {
...
- (void)previewTouchesBegan:(NSNotification *)notification {
    if (_selectedEffectCode) {
        ...
        [ShortVideoFilter.sharedInstance.filterProcessor addMediaEffect:_currentEffect];
...
- (IBAction)effectButtonPressed:(UIButton *)sender {
    ...
    if (code == [NSNull null]) {
        id effect = [ShortVideoFilter.sharedInstance.filterProcessor mediaEffectsWithType:TuSDKMediaEffectDataTypeParticle].lastObject;
        if (effect) {
            [ShortVideoFilter.sharedInstance.filterProcessor removeMediaEffect:effect];
