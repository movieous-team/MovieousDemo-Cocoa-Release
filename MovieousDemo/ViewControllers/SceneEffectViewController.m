...
@interface SceneEffectViewController ()
...
- (void)syncUIWithEffect {
    ...
    for (SceneEffect *effect in ShortVideoFilter.sharedInstance.sceneEffects) {
        UIColor *color;
...
- (void)setEffectStart:(id)effectCode color:(UIColor *)color {
    ShortVideoFilter *filter = [ShortVideoFilter sharedInstance];
    SceneEffect *effect = [SceneEffect new];
...
- (void)setEffectEnd {
    ShortVideoFilter *filter = [ShortVideoFilter sharedInstance];
    SceneEffect *effect = filter.sceneEffects.lastObject;
    NSTimeInterval effectDuration = _editor.currentTime - _currentEffectStartTime;
    if (effectDuration > 0) {
        // 修复误差
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    if (collectionView.tag == 0) {
        ...
        return kFUSceneEffectCodes.count + kTuSDKSceneEffectCodes.count + 1;
    }
}
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView.tag == 0) {
        ...
        if (indexPath.row == 0) {
            title = @"NO";
            imageName = @"noitem";
        } else if (indexPath.row <= kFUSceneEffectCodes.count) {
            title = kFUSceneEffectCodes[indexPath.row - 1];
            imageName = kFUSceneEffectCodes[indexPath.row - 1];
        } else {
            title = [NSString stringWithFormat:@"lsq_filter_%@", kTuSDKSceneEffectCodes[indexPath.row - kFUSceneEffectCodes.count - 1]];
            title = NSLocalizedString(title,@"特效");
            imageName = [NSString stringWithFormat:@"lsq_filter_thumb_%@.jpg",kTuSDKSceneEffectCodes[indexPath.row - kFUSceneEffectCodes.count - 1]];
        }
