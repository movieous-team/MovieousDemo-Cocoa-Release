//
//  MDImageGraffitiViewController.swift
//  MovieousDemo
//
...
protocol MDImageGraffitiViewControllerDelegate: NSObjectProtocol {
    func graffitiViewController(graffitiViewController: MDImageGraffitiViewController, didGetGraffiti snapshot: UIImage)
}

class MDImageGraffitiViewController: UIViewController {
    ...
    weak var delegate: MDImageGraffitiViewControllerDelegate?
    ...
    override func viewDidLoad() {
        ...
        self.graffitiView.isHidden = false
        ...
        self.brushSizeSlider.value = Float(self.graffitiView.brush.lineWidth)
        ...
        if self.graffitiView.canUndo() {
            self.undoButton.isEnabled = true
        } else {
            self.undoButton.isEnabled = false
        }
        if self.graffitiView.canRedo() {
    ...
    deinit {
        ...
        if let delegate = self.delegate {
            delegate.graffitiViewController(graffitiViewController: self, didGetGraffiti: self.graffitiView.takeSnapshot())
        }
    }
    
    @objc func sliderValueChanged(sender: UISlider) {
        self.graffitiView.brush.lineWidth = CGFloat(sender.value)
        if self.graffitiView.brush.lineColor == .clear {
            let image = resize(image: UIImage(named: "eraser")!, to: .init(width: self.graffitiView.brush.lineWidth, height: self.graffitiView.brush.lineWidth))
            self.brushSizeSlider.setThumbImage(image, for: .normal)
            self.brushSizeSlider.setThumbImage(image, for: .highlighted)
        } else {
            let image = createThumbImage(with: self.graffitiView.brush.lineWidth, color: self.graffitiView.brush.lineColor)
            self.brushSizeSlider.setThumbImage(image, for: .normal)
            self.brushSizeSlider.setThumbImage(image, for: .highlighted)
        }
    }
...
extension MDImageGraffitiViewController: MSVGraffitiViewDelegate {
    func graffitiViewUndoRedoStatusChanged(_ graffitiView: MSVGraffitiView) {
        if graffitiView.canUndo() {
            self.undoButton.isEnabled = true
        } else {
            self.undoButton.isEnabled = false
        }
        if graffitiView.canRedo() {
            self.redoButton.isEnabled = true
        } else {
            self.redoButton.isEnabled = false
        }
...
extension MDImageGraffitiViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if indexPath.row == 0 {
            self.graffitiView.brush.lineColor = .clear
            let image = resize(image: UIImage(named: "eraser")!, to: .init(width: self.graffitiView.brush.lineWidth, height: self.graffitiView.brush.lineWidth))
            self.brushSizeSlider.setThumbImage(image, for: .normal)
            self.brushSizeSlider.setThumbImage(image, for: .highlighted)
        } else {
            self.graffitiView.brush.lineColor = self.brushColors[indexPath.row - 1]
            let image = createThumbImage(with: self.graffitiView.brush.lineWidth, color: self.graffitiView.brush.lineColor)
            self.brushSizeSlider.setThumbImage(image, for: .normal)
            self.brushSizeSlider.setThumbImage(image, for: .highlighted)
        }
